...
class RCNN:
    def __init__(self, time, K, p, num_class, is_training=True):
        ...
        self.N = int(float(self.K)/8 + 1)
        self.alpha = 0.001/self.N
        self.depth_radius = (self.N - 1) /2  
        self.beta = 0.75
    ...
    def RCL(self, x, y, z):
        ...
        with slim.arg_scope([slim.conv2d], padding="SAME",
            ...
            conv1 = tf.nn.lrn(conv1, depth_radius = self.depth_radius, bias =1, alpha = self.alpha, beta = self.beta, name = "LRN1") 
          #  conv1 = slim.batch_norm(conv1, is_training=self.is_training, scope='batch1')
           #  listconv = conv1.get_shape().as_list()
            ...
            conv2 = tf.nn.lrn(conv2, depth_radius = self.depth_radius, bias=1, alpha = self.alpha, beta = self.beta, name = "LRN2")
            ...
            conv3 = tf.nn.lrn(conv3, depth_radius = self.depth_radius, bias = 1, alpha = self.alpha, beta = self.beta, name = "LRN3")
    ...
    def build_model(self, x):
        ...
        for i in range(2, 6):
            with tf.variable_scope('recurrent_conv{}'.format(i)) as scope:
                ...
                if i == 3:
                    output1 = slim.max_pool2d(output1, [3, 3],valid = "SAME", scope='pool1')
                    output2 = slim.max_pool2d(output2, [3, 3],valid = "SAME", scope='pool2')
                    output3 = slim.max_pool2d(output3, [3, 3],valid = "SAME", scope='pool3')
