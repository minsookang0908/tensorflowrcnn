...
with tf.Session() as sess:
   ...
   # Ybatches = np.split(np.asarray(Ytrain), 100);
        ...
        for i in range(500):  # 50000->500
            rcnn.mode(True)
            t, Loss = sess.run([train_step, cross_entropy], feed_dict={X: Xbatches[i], Y_: Ybatches[i], learning_schedule: lr})
            print(Loss, i)
        
        
            if i>= 399:
                rcnn.mode(False)
                Correct1,t, Loss = sess.run([correct,train_step, cross_entropy], feed_dict={X: Xbatches[i], Y_: Ybatches[i], learning_schedule: lr})
                print(Loss,i)
            
                Correct += Correct1
        present_accurcy =(float(Correct) /10000 * 100)
        if best_accuracy(1 + threshold) < present_accuracy:
            best_accuracy = present_accuracy
            count = 0
        else: 
            count +=1
